...
class Node
  ...
  def return_number_of_metadata_entries
    entries = @metadata_entries.sum
    if @number_of_children > 0
      @child_nodes.each do |node|
        entries += node.return_number_of_metadata_entries
      end
    end
    return entries
  end
...
puts root_node.return_number_of_metadata_entries
